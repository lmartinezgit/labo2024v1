...
# script correspondiente al experimento EXP2
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    learning_rate = c( 0.01, 0.5), #votación baseline
    max_depth = -1L, 
    feature_fraction = c( 0.2, 0.9),
    num_leaves = c( 8L, 2048L, "integer" ), #votación baseline
    min_data_in_leaf = c( 100L, 2000L, "integer" ), #votación baseline
     
    lambda_l1 = c(0.01, 100.0),
    lambda_l2 = c(0.01, 100.0),   
    bagging_fraction = c(0.5, 1.0), 
    min_sum_hessian_in_leaf = 0.001, 
    feature_fraction_bynode = c(0.01, 1.0),
    min_gain_to_split = c(0.0, 15.0),
    extra_trees = TRUE,
    use_quantized_grad = FALSE,  
    num_grad_quant_bins = c(-100L, 100L, "integer"),
    stochastic_rounding = FALSE,
    scale_pos_weight = c(1.0,100.0)

  )
   
  # una Beyesian de Guantes Blancos, con 50 iteraciones
...
#Previamente defino la variable para numerar los experimentos
...
corrida_guantesblancos_202109 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  DT_incorporar_dataset_default(paste0("DT0001", num_exp), "competencia_2024.csv.gz")
  CA_catastrophe_default(paste0("CA0001", num_exp),paste0("DT0001", num_exp))

  DR_drifting_guantesblancos(paste0("DR0001", num_exp),paste0( "CA0001", num_exp))
  FE_historia_guantesblancos(paste0("FE0001", num_exp),paste0( "DR0001", num_exp))

  TS_strategy_guantesblancos_202109(paste0("TS0001", num_exp),paste0( "FE0001",num_exp))

  HT_tuning_guantesblancos(paste0("HT0001",num_exp), paste0("TS0001",num_exp))

  # El ZZ depente de HT y TS
  ZZ_final_guantesblancos(paste0("ZZ0001",num_exp), c(paste0("HT0001", num_exp),paste0("TS0001",num_exp)))
...
corrida_guantesblancos_202107 <- function( pnombrewf, pvirgen=FALSE )
{
  ...
  TS_strategy_guantesblancos_202107(paste0("TS0002",num_exp),paste0("FE0001", num_exp))

  HT_tuning_guantesblancos(paste0("HT0002",num_exp),paste0("TS0002",num_exp))

  # El ZZ depente de HT y TS
  ZZ_final_guantesblancos(paste0("ZZ0002",num_exp), c(paste0("HT0002",num_exp),paste0("TS0002",num_exp)))
...
corrida_guantesblancos_202109(paste0( "gb01",num_exp) )


# Luego partiendo de  FE0001
# genero TS0002, HT0002 y ZZ0002

corrida_guantesblancos_202107(paste0( "gb02",num_exp))
